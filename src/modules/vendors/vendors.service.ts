...
export class VendorsService {
 ...
 async getVendorDashboard(
  ...
  const dailyRaw = await this.prisma.$queryRaw<{ date: string; count: string }[]>`
    ...
    ORDER BY date ASC
  `;
  const dailyRedemptions = dailyRaw.map(r => ({ date: r.date, count: Number(r.count) }));
...
async getOffersUsageHistory(userId: string) {
  ...
  const redemptionsByWeekday = weekdayLabels.map((day, i) => ({
    weekday: day,
    count: Number(redemptionsByWeekdayRaw.find(r => r.weekday === i)?.count ?? 0),
  }));
  ...
  // 5️⃣ Offer performance analysis
  const offerPerformanceAnalysis = offers.map(o => {
    const count = redemptionGroups.find(r => r.offerId === o.id)?._count.id ?? 0;
    let performance: 'LOW' | 'MEDIUM' | 'HIGH' = 'HIGH';
    if (count < 20) performance = 'LOW';
    else if (count <= 50) performance = 'MEDIUM';
