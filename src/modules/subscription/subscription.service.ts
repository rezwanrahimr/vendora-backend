...
export class SubscriptionService {
  ...
  private async getOrCreatePayment(
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      const existing = await tx.payment.findFirst({
        where: { idempotencyKey}, // need to add unique constraint on transactionId in DB for this to be fully safe
      });
  ...
  async subscribeAndCheckout(
    ...
    const payment = await this.getOrCreatePayment(user, plan, idempotencyKey);
